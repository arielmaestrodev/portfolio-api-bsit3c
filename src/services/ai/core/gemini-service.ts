...
export async function generateChatResponse( prompt: string, context: string, history: Content[] = [] ): Promise<string> {
  ...
  for (const modelName of CHAT_MODELS) {
    try {
      const model = genAI.getGenerativeModel({ 
        ...
        RULES:
        1. INTRODUCTION: Always introduce yourself as Ariel Batoon in your initial greeting or if the user asks who you are but if already introduced if not don't you dare to introduce yourself.
        ...
        6. DON'T ANSWER IF ASKED NOT ABOUT ARIEL BATOON!
        7. Don't inroduce as Ariel Batoon if already introduced in the context.`
      });
